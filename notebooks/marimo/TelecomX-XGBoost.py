...
@app.cell
def _(NUM_SEMENTE_ALEATORIA, XGBClassifier):
...
@app.cell
def _(X_train, model_xgb, y_train):
...
@app.cell
def _(X_test, model_xgb):
    # 8️⃣ Fazer previsões
    y_pred_xgb = model_xgb.predict(X_test)
    y_pred_proba_xgb = model_xgb.predict(X_test)
    return y_pred_proba_xgb, y_pred_xgb
...
@app.cell
def _(tml, y_pred_proba_xgb, y_pred_xgb, y_test):
...
@app.cell
def _(X_test, lt, pd, tml, y_pred_xgb, y_test):
    list_df = []
...
@app.cell
def _(NUM_SEMENTE_ALEATORIA, XGBClassifier):
    model_xgb_1 = XGBClassifier(eval_metric='logloss', random_state=NUM_SEMENTE_ALEATORIA)
    return (model_xgb_1,)


@app.cell
def _(GridSearchCV, model_xgb_1, param_grid):
    model_grid_xgb = GridSearchCV(estimator=model_xgb_1, param_grid=param_grid, scoring='recall', cv=5, n_jobs=-1, verbose=1)
    return (model_grid_xgb,)


@app.cell
def _(X_train, model_grid_xgb, y_train):
    model_grid_xgb.fit(X_train, y_train)
    return


@app.cell
def _(model_grid_xgb):
    model_grid_xgb.best_params_
    return


@app.cell
def _(X_test, model_grid_xgb):
    y_pred_grid_xgb = model_grid_xgb.predict(X_test)
    return (y_pred_grid_xgb,)


@app.cell
def _(X_test, model_grid_xgb):
    y_pred_proba_grid_xgb = model_grid_xgb.predict_proba(X_test)
    return (y_pred_proba_grid_xgb,)


@app.cell
def _(tml, y_pred_grid_xgb, y_pred_proba_grid_xgb, y_test):
